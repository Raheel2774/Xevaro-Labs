          ...
          {PACKAGES.map((pkg, i) => (
            <Reveal key={pkg.name} delay={(i % 3) * 0.08}>
              <div className={`relative flex h-full flex-col rounded-3xl border p-8 ${
                pkg.featured
                  ? 'border-[#B41D24]/40 bg-gradient-to-b from-[#7C1418]/[0.10] to-white/[0.02] shadow-[0_0_40px_rgba(180, 29, 36,0.12)]'
                  : 'border-white/10 bg-white/55'
              }`}>
                {pkg.featured && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-gradient-to-r from-[#7C1418] to-[#B41D24] px-4 py-1 font-mono text-[10px] uppercase tracking-widest text-white">
                    Most Popular
                  </span>
                )}
