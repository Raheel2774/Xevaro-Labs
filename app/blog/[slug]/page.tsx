    ...
    <>
      ...
      <article className="relative px-6 pb-24 pt-32 md:px-10">
        <div className="mx-auto max-w-3xl">
          ...
          <h1 className="mt-4 font-display text-4xl font-bold leading-tight tracking-tight text-white/90 md:text-5xl">
            {post.title}
          </h1>
          <p className="mt-5 text-lg font-light leading-relaxed text-white/60">{post.description}</p>

          <ArticleBody blocks={post.body} />

          {post.faqs?.length ? (
            <section className="mt-16">
              <h2 className="font-display text-2xl font-semibold text-white/90 md:text-3xl">Frequently asked questions</h2>
              <div className="mt-6 divide-y divide-white/[0.08] border-y border-white/[0.08]">
                {post.faqs.map((f, i) => (
                  <div key={i} className="py-5">
                    <h3 className="font-display text-lg font-medium text-white/85">{f.q}</h3>
                    <p className="mt-2 text-[15px] leading-relaxed text-white/55">{f.a}</p>
                  </div>
                ))}
              </div>
            </section>
