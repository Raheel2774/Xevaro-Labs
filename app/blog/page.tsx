    ...
    <>
      ...
      <section className="relative px-6 pb-10 pt-40 md:px-10">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h1 className="mt-5 font-display text-5xl font-bold tracking-tight text-[#FFFFFF]/90 md:text-7xl">
        ...
        <div className="mx-auto max-w-3xl divide-y divide-white/10 border-y border-white/10">
          {posts.map((p) => (
            <Link key={p.slug} href={`/blog/${p.slug}`} className="group block py-8">
              <div className="flex items-center gap-3 font-mono text-[11px] uppercase tracking-widest text-[#FFFFFF]/55">
              ...
              <h2 className="mt-3 font-display text-2xl font-semibold text-[#FFFFFF]/90 transition-colors group-hover:text-[#B41D24] md:text-3xl">
                {p.title}
              </h2>
              <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-[#FFFFFF]/60">{p.description}</p>
              <span className="mt-4 inline-flex items-center gap-2 text-sm text-[#FFFFFF]/55 transition-colors group-hover:text-[#B41D24]">
