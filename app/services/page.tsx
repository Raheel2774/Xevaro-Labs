    ...
    <>
      ...
      <section className="relative px-6 pb-16 pt-40 md:px-10">
        <div className="mx-auto max-w-7xl">
          ...
          <Reveal delay={0.1}>
            <p className="mt-6 max-w-2xl text-lg font-light text-[#FFFFFF]/65">
                ...
                <div onMouseMove={glow} className="h-full">
                  <GlassCard glow className="h-full p-8 md:p-10">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="rounded-full border border-white/15 px-3 py-1 font-mono text-[10px] uppercase tracking-wider text-[#FFFFFF]/55">{p.tagline}</span>
                    </div>
                    <h3 className="mt-6 font-display text-2xl font-semibold text-[#FFFFFF] md:text-3xl">{p.name}</h3>
                    <p className="mt-4 text-[15px] leading-relaxed text-[#FFFFFF]/65">{p.description}</p>

                    <ul className="mt-6 grid grid-cols-1 gap-2 sm:grid-cols-2">
                      {p.features.map((inc) => (
                        <li key={inc} className="flex items-center gap-2 text-sm text-[#FFFFFF]/60">
                    ...
                    <div className="mt-7 rounded-xl border border-white/10 bg-white/55 p-4">
                      ...
                      <p className="mt-1.5 text-sm text-[#FFFFFF]/75">{p.outcome}</p>
                    </div>
                  </GlassCard>
                </div>
              </Reveal>
          ...
          <Reveal>
            <div className="relative overflow-hidden rounded-3xl border border-[#B41D24]/20 bg-gradient-to-br from-[#7C1418]/[0.08] via-white/[0.02] to-transparent p-8 md:p-12">
              ...
              <div className="relative flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
                <div>
                  <h3 className="max-w-xl font-display text-2xl font-semibold text-[#FFFFFF] md:text-3xl">
                    Want a ready made agent for your industry?
                  </h3>
                  <p className="mt-3 max-w-lg text-sm text-[#FFFFFF]/65">
            ...
            {PROCESS.map((p, i) => (
              <Reveal key={p.step} delay={(i % 4) * 0.08}>
                <div className="relative h-full rounded-2xl border border-white/[0.07] bg-white/55 p-7">
                  ...
                  <h3 className="mt-3 font-display text-xl font-semibold text-[#FFFFFF]">{p.title}</h3>
            ...
            {DIFFERENTIATORS.map((d, i) => (
              <Reveal key={d.title} delay={(i % 2) * 0.08}>
                <div className="flex h-full gap-4 rounded-2xl border border-white/[0.07] bg-white/55 p-7">
                  ...
                  <div>
                    <h3 className="font-display text-lg font-semibold text-[#FFFFFF]">{d.title}</h3>
                    <p className="mt-2 text-sm leading-relaxed text-[#FFFFFF]/65">{d.desc}</p>
                  </div>
                </div>
              </Reveal>
            ))}
      ...
      <section className="relative px-6 py-24 text-center md:px-10">
        ...
        <Reveal delay={0.08}>
          <p className="mx-auto mt-5 max-w-xl text-[#FFFFFF]/65">
