    ...
    <>
      <section className="relative px-6 pb-20 pt-40 md:px-10">
        <div className="mx-auto max-w-7xl">
          ...
          <Reveal delay={0.1}>
            <p className="mt-7 max-w-2xl text-lg font-light leading-relaxed text-[#FFFFFF]/65">
            ...
            <div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/[0.06] md:grid-cols-4">
              {TRUST_STATS.map((s) => (
                <div key={s.label} className="bg-white/[0.06] p-8 text-center backdrop-blur-xl">
                  ...
                  <div className="mt-2 text-xs uppercase tracking-wide text-[#FFFFFF]/55">{s.label}</div>
                </div>
              ))}
            </div>
          </Reveal>
          ...
          {[
            ...
            <Reveal key={t} delay={i * 0.1}>
              <div className="rounded-2xl border border-white/10 bg-white/55 p-8 backdrop-blur-xl">
                ...
                <h3 className="mt-4 font-display text-2xl font-semibold text-[#FFFFFF]/90">{t}</h3>
                <p className="mt-3 text-sm leading-relaxed text-[#FFFFFF]/60">{d}</p>
              </div>
            </Reveal>
          ))}
        </div>
            ...
            {TIMELINE.map((t, i) => (
              <Reveal key={t.year} delay={i * 0.08}>
                <div className="group flex gap-8 border-l border-white/15 py-7 pl-8 transition-colors hover:border-[#B41D24]/50">
                  ...
                  <div>
                    <h3 className="font-display text-xl font-semibold text-[#FFFFFF]/90">{t.title}</h3>
                    <p className="mt-1.5 text-sm text-[#FFFFFF]/60">{t.desc}</p>
                  </div>
                </div>
              </Reveal>
            ))}
            ...
            {DIFFERENTIATORS.map((d, i) => (
              <Reveal key={d.title} delay={(i % 2) * 0.08}>
                <div className="flex h-full gap-4 rounded-2xl border border-white/10 bg-white/55 p-7">
                  ...
                  <div>
                    <h3 className="font-display text-lg font-semibold text-[#FFFFFF]/90">{d.title}</h3>
                    <p className="mt-2 text-sm leading-relaxed text-[#FFFFFF]/60">{d.desc}</p>
                  </div>
                </div>
              </Reveal>
            ))}
