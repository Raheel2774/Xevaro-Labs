    ...
    <>
      <section className="relative px-6 pb-8 pt-32 md:px-10">
        <div className="mx-auto max-w-7xl">
          ...
          <Pop delay={0.05}>
            <h1 className="max-w-3xl font-display text-4xl font-bold leading-tight tracking-tight text-[#5D001E] md:text-6xl">
              Six AI automation products. One unified platform.
            </h1>
          </Pop>
          <Pop delay={0.1}>
            <p className="mt-6 max-w-2xl text-lg font-light leading-relaxed text-[#5D001E]/65">
                  ...
                  <Link href={`/products/${p.id}`} className="flex h-full flex-col">
                    ...
                    <div className="mt-5 flex flex-wrap gap-2">
                      {p.stats.slice(0, 3).map(s => (
                        <span key={s.label} className="rounded-full bg-[#E3E2DF] px-3 py-1 text-xs font-semibold text-[#5D001E]"><span className="text-[#EE4C7C]">{s.value}</span> {s.label}</span>
                      ))}
                    </div>
      ...
      <section className="relative px-6 py-20 md:px-10">
        <div className="mx-auto max-w-7xl">
          <Pop><h2 className="font-display text-3xl font-bold text-[#5D001E] md:text-5xl">All pricing, in one place.</h2></Pop>
          <Pop delay={0.05}>
            <div className="mt-10 overflow-x-auto rounded-[28px] border border-[#5D001E]/10 bg-white/60">
                  ...
                  {PRODUCTS.map((p) => (
                    <tr key={p.id} className="border-b border-[#5D001E]/10 transition-colors last:border-0 hover:bg-[#E3AFBC]/20">
                      <td className="px-5 py-4">
                        <Link href={`/products/${p.id}`} className="font-semibold text-[#5D001E] hover:text-[#9A1750]">{p.name}</Link>
      ...
      <section className="relative px-6 py-20 md:px-10">
        <div className="mx-auto max-w-3xl text-center">
          <Pop><h2 className="font-display text-4xl font-bold text-[#5D001E] md:text-5xl">Ready to automate?</h2></Pop>
          <Pop delay={0.1}>
