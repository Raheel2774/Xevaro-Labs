      ...
      <section className="relative px-6 pb-12 pt-28 md:px-10">
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
            ...
            <div className="relative">
              <div className="flex items-center gap-3">
                <span className="flex h-14 w-14 items-center justify-center rounded-2xl text-white shadow-lg"
                  style={{ background: `linear-gradient(135deg, ${product.accent}, #B41D24)` }}>
                  <Icon className="h-7 w-7" />
                </span>
                <span className="font-mono text-xs uppercase tracking-[0.3em] text-[#E2A5A5]">Product {product.index}</span>
              </div>
              <h1 className="mt-6 font-display text-4xl font-bold leading-tight text-white md:text-6xl">{product.name}</h1>
              <p className="mt-3 text-xl font-medium text-[#B41D24]">{product.tagline}</p>
              <p className="mt-6 max-w-xl text-lg font-light leading-relaxed text-[#E2A5A5]/85">{product.longDescription}</p>
          ...
          <motion.div
            ...
            className="relative flex flex-col justify-between rounded-[32px] border border-white/10 bg-white/75 p-8 shadow-[0_12px_44px_rgba(124, 20, 24,0.14)] backdrop-blur-xl"
          >
            <Sticker className="absolute -top-3 right-6 bg-[#B41D24] text-white" rotate={5}>{product.badge}</Sticker>
            <div>
