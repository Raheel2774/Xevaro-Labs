    ...
    <>
      ...
      <section className="relative px-6 pb-12 pt-28 md:px-10">
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-5 lg:grid-cols-[1.4fr_1fr]">
          ...
          <motion.div
            ...
            className="relative flex flex-col justify-between rounded-[32px] border border-[#5D001E]/10 bg-white/75 p-8 shadow-[0_12px_44px_rgba(154,23,80,0.14)] backdrop-blur-xl"
          >
            ...
            <div>
              ...
              <div className="mt-6 space-y-2.5 border-t border-[#5D001E]/10 pt-5">
                {[['Year 1 investment', `$${year1.toLocaleString()}`], ['Deployment', '48 hours'], ['Contract', 'Month to month']].map(([k, v]) => (
                  <div key={k} className="flex justify-between text-sm">
                    <span className="text-[#5D001E]/60">{k}</span>
                    <span className="font-bold text-[#5D001E]">{v}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="mt-7">
              ...
              <p className="mt-4 text-center text-xs text-[#5D001E]/50">No long contracts. Cancel anytime.</p>
            </div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-5 sm:grid-cols-3">
          {product.stats.map((s, i) => (
            <BentoCard key={s.label} delay={i * 0.08} className="p-8 text-center">
              ...
              <div className="mt-2 text-sm uppercase tracking-wide text-[#5D001E]/55">{s.label}</div>
            </BentoCard>
          ))}
        </div>
      </section>
      ...
      <section className="relative px-6 py-16 md:px-10">
        <div className="mx-auto max-w-7xl">
          <Pop><h2 className="font-display text-3xl font-bold text-[#5D001E] md:text-5xl">How it works</h2></Pop>
          <div className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {product.howItWorks.map((step, i) => (
              <BentoCard key={step.title} delay={i * 0.08} accent={product.accent} className="p-7">
                ...
                <h3 className="mt-4 font-display text-lg font-bold text-[#5D001E]">{step.title}</h3>
                <p className="mt-2 text-sm leading-relaxed text-[#5D001E]/65">{step.desc}</p>
              </BentoCard>
            ))}
          </div>
        </div>
      ...
      <section className="relative px-6 py-8 md:px-10">
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-5 lg:grid-cols-2">
          <BentoCard className="p-8" accent={product.accent}>
            <h3 className="font-display text-2xl font-bold text-[#5D001E]">What you get</h3>
            ...
            <div className="mt-8 rounded-2xl bg-[#E3AFBC]/25 p-5">
              ...
              <p className="mt-1.5 text-lg font-semibold text-[#5D001E]">{product.outcome}</p>
            </div>
          </BentoCard>

          <BentoCard className="p-8">
            <h3 className="font-display text-2xl font-bold text-[#5D001E]">Perfect for</h3>
            ...
            <h3 className="mt-10 flex items-center gap-2 font-display text-2xl font-bold text-[#5D001E]"><Plug className="h-5 w-5 text-[#EE4C7C]" /> Integrates with</h3>
            <div className="mt-6 flex flex-wrap gap-2.5">
              {product.integrations.map((int, i) => (
                <motion.span key={int} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }} transition={{ delay: i * 0.05 }}
                  className="rounded-xl border border-[#5D001E]/10 bg-white px-4 py-2 text-sm font-medium text-[#5D001E]/75">{int}</motion.span>
              ))}
            </div>
          </BentoCard>
        </div>
      ...
      <section className="relative px-6 py-16 md:px-10">
        <div className="mx-auto max-w-4xl">
          <Pop><h2 className="font-display text-3xl font-bold text-[#5D001E] md:text-5xl">Questions, answered</h2></Pop>
          <div className="mt-8 space-y-4">
            {product.faq.map((f, i) => (
              <BentoCard key={f.q} delay={i * 0.06} tilt={false} className="p-6">
                <h3 className="font-display text-lg font-bold text-[#5D001E]">{f.q}</h3>
                <p className="mt-2 text-sm leading-relaxed text-[#5D001E]/65">{f.a}</p>
              </BentoCard>
            ))}
          </div>
        </div>
      ...
      {/* ── Other products ── */}
      <section className="relative border-t border-[#5D001E]/10 px-6 py-20 md:px-10">
        <div className="mx-auto max-w-7xl">
          <Pop><h2 className="font-display text-3xl font-bold text-[#5D001E] md:text-4xl">Pairs well with</h2></Pop>
              ...
              return (
                <BentoCard key={p.id} delay={i * 0.08} accent={p.accent} className="p-6">
                  <Link href={`/products/${p.id}`} className="flex h-full flex-col">
                    ...
                    <p className="mt-2 flex-1 text-sm text-[#5D001E]/60">{p.tagline}</p>
                    <span className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-[#5D001E] group-hover:text-[#EE4C7C]">View <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span>
                  </Link>
                </BentoCard>
              )
            })}
