...
export default function ProofStrip() {
  ...
  return (
    <section className="relative border-y border-[#5D001E]/10 bg-white/40 py-10">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        ...
        <div className="relative mt-7 flex overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
          <div className="flex shrink-0 animate-marquee items-center gap-12 pr-12">
            {items.map((name, i) => (
              <span key={i} className="whitespace-nowrap font-display text-xl font-semibold text-[#5D001E]/50 transition-colors hover:text-[#9A1750]">
                {name}
              </span>
            ))}
          </div>
