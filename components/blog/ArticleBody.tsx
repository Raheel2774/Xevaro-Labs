...
function renderInline(text: string): React.ReactNode[] {
  ...
  while ((m = regex.exec(text)) !== null) {
    ...
    if (m[1] !== undefined) {
      nodes.push(
        <strong key={key++} className="font-semibold text-white/90">{m[1]}</strong>,
      )
    } else {
      const label = m[2]
      const href = m[3]
  ...
  return (
    <div className="mt-10 space-y-5">
      {blocks.map((b, i) => {
        switch (b.type) {
          case 'h2':
            return <h2 key={i} className="pt-6 font-display text-2xl font-semibold text-white/90 md:text-3xl">{renderInline(b.text)}</h2>
          case 'h3':
            return <h3 key={i} className="pt-2 font-display text-lg font-semibold text-white/85 md:text-xl">{renderInline(b.text)}</h3>
          case 'p':
